...
# ================================================================
APServiceConfigDirectory=$PENTWorkspacePath
# ================================================================
...
function ap_service_stop() {
  if [ "$APServicePID" ]; then
    kill $APServicePID &> $PENTOutputDevice
  fi
...
function ap_service_reset() {
  ...
  macchanger -p $APServiceInterface &> $PENTOutputDevice
  sleep 0.25
...
function ap_service_route() {
  echo "APService: No custom routes for hostapd" > $PENTOutputDevice
}
...
channel=$APServiceChannel" \
  ...
  macchanger --mac=$APServiceMAC $APServiceInterface &> $PENTOutputDevice
  sleep 0.5
...
function ap_service_start() {
  ...
  xterm $PENTHoldXterm $TOP -bg "#000000" -fg "#FFFFFF" \
    -title "Wifipot AP Service [hostapd]" -e \
