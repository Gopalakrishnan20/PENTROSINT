...
if [ -f "/etc/debian_version" ]; then
  ...
  unprep_package_manager() {
    echo "$(cat /etc/apt/sources.list | grep -v 'deb http://http.kali.org/kali kali-rolling main non-free contrib # Installed By PENT')" >/etc/apt/sources.list
  }
  ...
  prep_package_manager() {
    if [ ! "$(cat /etc/apt/sources.list | egrep 'deb http://http.kali.org/kali ((kali-rolling|main|contrib|non-free) )*')" ]; then
      ...
      echo "deb http://http.kali.org/kali kali-rolling main non-free contrib # Installed By PENT" >>/etc/apt/sources.list
    fi
